...
def create_patient_table():
    try:
        cursor = db.cursor()
        sql = "CREATE TABLE patient (patientId int AUTO_INCREMENT PRIMARY KEY, patientName varchar(255) NOT NULL, phone int, dentistId int, FOREIGN key (dentistId) REFERENCES dentist(dentistId))"
        cursor.execute(sql)
